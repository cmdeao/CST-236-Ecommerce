...
class UserDataService
{
    ...
    function storeRedeemedCoupon($couponID)
    {
        $database = dbConnection();
        $userID = getUserID();
        
        $query = "SELECT REDEEMED_COUPONS FROM users WHERE USER_ID = '$userID'";
        $result = mysqli_query($database, $query);
        
        if(mysqli_num_rows($result) == 1)
        {
            $row = $result->fetch_assoc();
            $usedCoupons = $row["REDEEMED_COUPONS"];
        }
        
        if(!empty($usedCoupons))
        {
            $intIDS = array_map('intval', explode(',',$usedCoupons));
            if(!in_array($couponID, $intIDS))
            {
        ...
        $query = $database->prepare("UPDATE users SET REDEEMED_COUPONS = '$usedCoupons' WHERE USER_ID = '$userID'");
        $query->execute();
